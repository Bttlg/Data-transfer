  ...
  <div>
    <div class="container">
      <b-tabs v-model="activeTab" type="is-boxed" expanded>
        <b-tab-item label="Файлууд" v-bind:class="{ noanim: !$store.state.settings.anim }">
          <template slot="header">
            <file-multiple-icon class="icon is-small"></file-multiple-icon>
            <span><span class="icon-text">Файлууд</span> <b-tag class="countTag" v-bind:class="{ 'is-danger': glowFilesBtn }" rounded>{{ shares.length }}</b-tag> </span>
          </template>
          <div class="field content is-grouped is-grouped-multiline">
            <div class="control">
              <b-upload v-model="files" :multiple="true" @input="onFileChange">
                <a class="button is-primary" aria-label="Файл/Хавтас нэмэх" title="Файл/Хавтас нэмэх">
                  <file-upload-icon class="icon is-small"></file-upload-icon>
                  <span class="icon-text">Файл нэмэх</span>
                </a>
              </b-upload>
            </div>
              ...
              <div class="control" v-if="tableCheckedRows.length === 1">
                <b-field grouped>
                  <div class="control" v-if="tableCheckedRows[0].paused">
                    <a class="button is-warning" @click="resumeShare" aria-label="Үргэлжлүүлэх" title="Үргэлжлүүлэх">
                      <play-icon class="icon is-small"></play-icon>
                    </a>
                  </div>
                  <div class="control" v-else>
                    <a class="button" @click="pauseShare" aria-label="Завсарлах" title="Завсарлах">
                      <pause-icon class="icon is-small"></pause-icon>
                    </a>
                  </div>
                  <div class="control">
                    <a class="button is-danger" @click="removeShare" aria-label="Устгах" title="Устгах">
          ...
          <b-table
            ...
            <template slot-scope="props">
              <b-table-column field="name" label="Нэр" width="40vw" v-bind:class="{ 'is-warning' : props.row.paused }" sortable>
                <span style="word-break: break-word;max-width: 60vw;">{{ props.row.name }}</span>
              </b-table-column>
              <b-table-column field="size" label="Хэмжээ" width="10vw" v-bind:class="{ 'is-warning' : props.row.paused }" sortable>
                {{ props.row.size }}
              </b-table-column>
              <b-table-column field="stats" label="Төлөв" width="50vw">
                <div v-show="!props.row.mine" class="control is-expanded downloadSection">
                  <a v-show="props.row.done" v-bind:href="props.row.downloadURL" v-bind:download="props.row.name">
                    <b-button type="is-success">Татаж авах</b-button>
                  </a>
                  <b-progress v-show="!props.row.done" type="is-success" size="is-medium" :value="props.row.progress" show-value>
                    ...
                    <span v-show="props.row.paused" class="has-text-black">&nbsp;(Завсарласан)</span>
                  </b-progress>
                </div>
                <b-field grouped group-multiline v-if="props.row.mine">
                  <div class="control">
                    <span class="tags has-addons">
                      <span class="tag is-info"><upload-icon class="icon is-small" title="Бусад төхөөрөмжөөс татаж авахад бэлэн"></upload-icon></span>
                    </span>
                  </div>
            ...
            <template slot="empty">
              <b-upload v-model="files" @input="onFileChange" :multiple="true">
                <p id="drop-area">Файлуудаа энд чирж оруулах эсвэл энд дарж файлаа сонгоно уу?</p>
              </b-upload>
              <center>
                <p>Та <a href="https://a-z.mn">A-Z сайт</a>-ыг хүлээн авагчдаа нээж нэг өрөөнд орох хэрэгтэй. Нэг дотоод сүлжээнд байгаа бол автоматаар нэг өрөөнд орно.</p>
                <p>Интернетээр дамжуулах хэрэгтэй юу ? <router-link to="/room">Өрөө үүсгэж ашиглаарай.</router-link></p>
              </center>
            </template>
          </b-table>
        </b-tab-item>
        <b-tab-item label="Мессэж" v-bind:class="{ noanim: !$store.state.settings.anim }">
          <template slot="header">
            <android-messages-icon class="icon is-small"></android-messages-icon>
            <span><span class="icon-text">Мессеж</span> <b-tag class="countTag" v-bind:class="{ 'is-danger': glowMsgsBtn }" rounded>{{ msgs.length }}</b-tag></span>
          </template>
          <b-field label="Мессэж бичих" class="is-floating-label">
            <div class="control is-clearfix">
              <textarea
                placeholder="Энд текст бичнэ үү..."
                class="textarea"
                v-model="msg"
                ref="msgInput"
          ...
          <b-field>
            <div class="control">
              <b-button type="button is-primary" @click="sendMsg">Илгээх</b-button>
              <div class="is-pulled-right">
                <b-tooltip label="Автоматаар хуулах" position="is-left" type="is-dark">
                  <router-link to="/settings" tag="div" class="button is-dark">
                    <robot-icon class="icon is-small"></robot-icon>
                  </router-link>
                </b-tooltip>&nbsp;
                <b-tooltip label="Бүх мессежийг цэвэрлэх" position="is-left" type="is-dark">
                  <a class="button is-danger" aria-label="Бүх мессежийг цэвэрлэх" title="Бүх мессежийг цэвэрлэх" @click="clearAllMsgs">
          ...
          <div id="messages">
            ...
            <div class="card" v-for="(msg, index) in msgs" :key="index">
              <header class="card-header">
                ...
                <a class="card-header-icon" title="Мессежийг хуулах" @click="copyMsg" v-clipboard="msg.msg">
                  Хуулах
                </a>
              </header>
        ...
        <b-tab-item label="Төхөөрөмжүүд" v-bind:class="{ noanim: !$store.state.settings.anim }">
          <template slot="header">
            <devices-icon class="icon is-small"></devices-icon>
            <span><span class="icon-text">Төхөөрөмжүүд</span> <b-tag class="countTag" v-bind:class="{ 'is-danger': glowUsersBtn }" rounded>{{ usersCount }}</b-tag></span>
          </template>
          <div class="content" style="position: relative;border-bottom: 2px dashed #dbdbdb;">
            <p>
              <router-link to="/grid">
                <b-button type="is-primary" style="float: right;">A-Z Grid ашиглах</b-button>
              </router-link>
              <b-taglist attached>
                <router-link to="/settings">
                  ...
                  <b-tag type="is-warning" size="is-medium">Энэ таны төхөөрөмж</b-tag>
                </router-link>
              </b-taglist>
            </p>
            <span style="position: absolute;bottom: -11px">Төхөөрөмжүүд</span>
            <!-- empty <p> necessary -->
            <p></p>
          </div>
  ...
  mounted () {
    ...
    this.$store.subscribeAction(action => {
      ...
      if (type === 'uploadProgress') {
        ...
        this.$buefy.toast.open({
          duration: 2000,
          message: 'Өрөөний код буруу байна',
          position: 'is-top',
          type: 'is-danger',
          queue: false
        })
