    ...
    <b-navbar v-if="$route.name !== 'About'" class="is-success has-shadow" :mobile-burger="false" :transparent="true">
      <template slot="brand">
        <b-navbar-item id="brand" tag="router-link" :to="{ path: '/' }">
          <h1 class="is-size-4">A-Z файл дамжуулалт</h1>
        </b-navbar-item>
        <div class="actions">
          <b-tooltip :label="internetShare ? 'Интернетээр дамжуулах өрөөнд орсон' : 'Интернетээр дамжуулах'" position="is-bottom" :type="internetShare ? 'is-info' : 'is-primary'">
            <b-navbar-item tag="router-link" :to="{ path: '/room' }">
              <a class="button is-text" v-bind:class="{ 'is-info' : internetShare }" :aria-label="internetShare ? 'Интернетээр дамжуулах өрөөнд орсон' : 'Интернетээр дамжуулах'">
                <earth-icon class="icon is-medium"></earth-icon>
              </a>
            </b-navbar-item>
          </b-tooltip>
          <b-tooltip label="Тохиргоо" position="is-bottom" type="is-dark">
          ...
          <b-navbar-item tag="router-link" :to="{ path: '/about' }">
            <b-button type="is-warning">Тусламж</b-button>
          </b-navbar-item>
        </div>
      </template>
    </b-navbar>
