...
export default {
  ...
  methods: {
      ...
      if (roomID && this.$validateRoomCode(roomID)) {
        ...
        try {
          ...
          this.$buefy.snackbar.open({
            message: 'Таны IP хаягийг олж чадахгүй байна',
            position: 'is-top',
            type: 'is-danger',
            queue: true,
            indefinite: true,
            actionText: 'Дахин оролдох',
    ...
    startP2PT (identifier) {
      ...
      p2pt.setIdentifier('azmn' + identifier)
      ...
      p2pt.on('trackerwarning', (error, stats) => {
        ...
        if (warningCount >= stats.total && !trackerConnected && !warningMsg) {
          warningMsg = this.$buefy.snackbar.open({
            message: 'Холболт үүсгэж чадахгүй байна. Таны интернетийн p2p холболт хаалттай юу?',
            position: 'is-top',
            type: 'is-danger',
            queue: false,
            indefinite: true,
